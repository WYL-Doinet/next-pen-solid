...
export default function EditorBottomMenu(props: EditorBottomMenuProps) {
    ...
    return <div class="py-2.5 px-3 flex items-center gap-2">
                ...
                {(item) => {
                    switch (item.type) {
                        case NextMode.ERASER:
                            return <DropdownMenu open={openMenu() === NextMode.ERASER} placement="top" onOpenChange={(value) => {
                                if (value) {
                                ...
                                <DropdownMenuTrigger
                                    as={(menuProps: DropdownMenuSubTriggerProps) => (
                                        <Button variant={props.mode === item.type ? 'default' : 'ghost'} size={'sm'} {...menuProps}>
                                            {item.icon}
                                        </Button>
                                    )}
                                />
                                    ...
                                    <DropdownMenuGroup >
                                        <DropdownMenuGroupLabel class="space-y-1">
                                            ...
                                            <div class="flex items-center gap-2.5">
                                                <input value={eraserSize()} max={100} oninput={(e) => {
                                                    handleEraserSize(parseInt(e.target.value, 10))
                                                }} type="range" class="bg-gray-200  w-35 rounded-lg h-2 appearance-none cursor-pointer slider" />
                        ...
                        case NextMode.PENCIL:
                            return <DropdownMenu placement="top" open={openMenu() === NextMode.PENCIL} onOpenChange={(value) => {
                                if (value) {
                                ...
                                <DropdownMenuContent class="w-56">
                                    <DropdownMenuGroup >
                                        <DropdownMenuGroupLabel class="flex items-center gap-2.5">
                                            <span >Pick color</span>
                                            <div class="flex relative  gap-2 items-center flex-1 h-5 rounded-md" style={{ "background-color": pencilColor() }}>

                                                <input class="absolute top-0 left-0 w-full h-full opacity-0" oninput={(e) => {
                                        ...
                                        <For each={colors}>
                                            {(item) => <DropdownMenuItem >
                                                <div class="flex gap-2.5" onclick={() => handlePencilColor(item.hex)}>
                                                    <div class="size-5" style={{ "background-color": item.hex }} ></div>
                                    ...
                                    <DropdownMenuGroup>
                                        <DropdownMenuGroupLabel class="space-y-1">
                                            <div>Pick Size</div>
                                            <div class="flex items-center gap-2.5">
                                                <input value={pencilSize()} max={100} oninput={(e) => {
                        ...
                        default:
                            return <Button onclick={() => props.onModeChange(item.type)} variant={props.mode === item.type ? 'default' : 'ghost'} size={'sm'}>
                                {item.icon}
                            </Button>;
                    }
                }}
        ...
        <div class="p-2 rounded-md bg-gary-50 bg-gray-100 text-gray-700 flex gap-1">
            <For each={nextShape}>
                {(item) => <Button onclick={() => props.onShapeAdd(item.type)} variant={'ghost'} size={'sm'}>
                    {item.icon}
                </Button>}
            </For>
        </div>
